...
你必須優先掃描目前的【對話紀錄 history】與【最新輸入】，檢查以下資訊是否齊全：
1. 【關係脈絡】：對象是誰？(如：另一半、長輩、主管)。
2. 【衝突情境】：具體發生什麼事？(如：被指責動作慢、已讀不回)。
3. 【社交目標】：使用者想達成什麼？(如：道歉、優雅反擊)。
...
{{
  ...
  "suggested_scenarios": [
    {{
      "title": "按鈕標題",
      "example": "按鈕對應的文字內容"  # ✨ 統一使用 example，不要用 content
    }}
  ],
  "key_change": "💡 核心洞察",
  "analysis": "分析內容",
